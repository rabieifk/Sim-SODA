...
/*redirected avf output file name, by Xin Fu */
static char *sim_avfout=NULL;
FILE *sim_avffd = NULL;
int datadump;
...
banner(FILE *fd, int argc, char **argv)
{
  ...
  fprintf(fd,
	  "sim-alpha version %d.%d of %s.\n"
	  "Copyright (c) 1999 - 2001 by Raj Desikan.  All Rights Reserved.\n"
...
{
  lq_sq_end(commit_sq,commit_sq_nelem);
  lq_sq_end(issue_lq,issue_lq_nelem);
  lq_sq_sum(commit_sq,commit_sq_nelem,1);
  lq_sq_sum(issue_lq,issue_lq_nelem,0);
  inform_dump();
  address_based_AVF_dump();
  /* print simulation stats */
...
m=strlen(argv[argc-1]);

/* the number of instructions to fastfwd, by Xin Fu */
fastfwd_number=0;
for(n=0;n<m;n++)
	{
	  fastfwd_number=fastfwd_number*10+(argv[argc-1][n]-48);
	}
argv[argc-1]="";
argc--;
  ...
  opt_reg_string(sim_odb, "-redir:avf",
		 "redirect avf output to file",
		 &sim_avfout, /* default */NULL, /* !print */FALSE, NULL);
 ...
 if(sim_avfout != NULL)
	{
	 /*redirect simulated avf output to file SIM_AVFOUT, by Xin Fu */
	 sim_avffd = fopen(sim_avfout, "w");
	 if(!sim_avffd)
       fatal("unable to redirect avf output to file `%s'", sim_avfout);
    }
