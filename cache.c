 ...
 * Rajagopalan Desikan as part of the 21264 validation project.
 ...
 * 
 * Copyright (C) 1999 by Rajagopalan Desikan
 *
 ...
 *
 * $Id: cache.c,v 2.9 2000/05/09 01:29:40 raju Exp $
 *
 * $Log: cache.c,v $
 * Revision 2.9  2000/05/09 01:29:40  raju
 * Contains more support for load/store queue. The memory access instructions
 * are handled separately. Paricularly, load instructions are executed using
 * using event handler. Is executing incorrectly. Source of problem seems to
 * be 11th LDQ instruction which is not getting the correct value from memory
 * even though the address is correct. Baffling!!!!!
 *
 * Revision 2.8  2000/05/05 21:02:29  raju
 * Contains partial support for LQ/SQ with the memory accesses separated
 * in the writeback stage
 *
 * Revision 2.7  2000/05/04 20:19:47  raju
 * Contains more partial support for loads/stores
 *
 * Revision 2.6  2000/05/03 18:08:26  raju
 * Contains memory hierarchy options and partial support for load store queue
 *
 * Revision 2.5  2000/04/28 21:07:06  raju
 * Contains working microarchitecture/functional simulator with the new memory
 * structure
 *
 * Revision 2.2  2000/03/27 20:20:55  raju
 * Contains working microarch simulator w/o memory support
 *
 * Revision 1.2  1999/09/08 19:37:50  raju
 * Fuctional simulator now working. Processes fetch queue number of inst.
 *
 * Revision 1.1  1999/08/23 19:52:52  raju
 * Initial revision
 *
